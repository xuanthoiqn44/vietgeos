...
//get image featured article
if(isset($attribs->spfeatured_image) && $attribs->spfeatured_image != '') {

	if($imgsize == 'default') {
		$intro_image1 = $attribs->spfeatured_image;
	} else {
		$intro_image1 = $attribs->spfeatured_image;
		$basename = basename($intro_image1);
		$list_image = JPATH_ROOT . '/' . dirname($intro_image1) . '/' . JFile::stripExt($basename) . '_'. $imgsize .'.' . JFile::getExt($basename);
		if(file_exists($list_image)) {
			$intro_image1 = JURI::root(true) . '/' . dirname($intro_image1) . '/' . JFile::stripExt($basename) . '_'. $imgsize .'.' . JFile::getExt($basename);
		}
	}
}*/
	...
	{	
		...
		if(!is_null($images)) {
			...
			elseif (isset($images->image_fulltext)) {
				...
				//$introImage = htmlspecialchars($intro_image1);
			}
		}
		if ( !empty($introImage) ) {
